...
**Learning Objectives:**
- Identify each component in the Honeycomb Web SDK architecture (e.g., user events, Honeycomb Web SDK, OTel Collector > OTLP > exporter)
- Trace the path of a user interaction from the browser to Honeycomb ingest
- Pinpoint where to inject custom attributes
...
**Audio:** "When a user clicks the 'Go' button or enters custom text in Meminator, these actions trigger DOM events in the browser. Observability is a way we can captures what our system's users do so we can understand how our app behaves in the real world. The Honeycomb Web SDK automatically detects these interactions and starts creating what we call 'spans'—like breadcrumbs that show the journey of each user action."
...
**Audio:** "The Honeycomb Web SDK is like adding a lens to your React app. When Meminator runs, the SDK automatically sets up instrumentation. It tracks page loads, button clicks, API calls, and even errors.  You don't need to manually add tracking code everywhere; the SDK handles most of it automatically."

## 3. What is OpenTelemetry?
**Diagram Element:** OTel WebSDK diamond component connected to the React frontend.  
**Audio:** "You might wonder: what's OpenTelemetry? Think of it as..[use the same definition in OTel Foundations course]. The Honeycomb Web SDK uses OpenTelemetry "under the hood", which means your traces can connect seamlessly from your React frontend to your backend services.
...
**Audio:** "When you click 'Go' in Meminator, the SDK creates what's called a 'span'—think of it as a unit of work that's measured by how long it takes. This first span represents the user's action and becomes the 'root' of a trace. A trace is like a family tree showing everything that happens because of that one button click."
...
**Audio:** "As Meminator calls different backend services—one for images, one for text, for example—that same trace ID flows through each service. This creates a complete picture: you can see not just that a meme generation was slow, but exactly which service caused it to break. However, this only works if your backend services are configured to read and forward those trace headers."

## 7. When Things Go Wrong
**Diagram Element:** Error icon connected to a span showing failed API call with error details.  
**Audio:** "Real-world applications have errors and failures. When something goes wrong in Meminator—like a meme failing to load—the SDK captures these errors as part of the trace. Instead of just seeing 'meme generation failed,' you can see exactly where and why it failed, making debugging much easier."
...
**Audio:** "The exporter is like the shipping department for your trace data. It's configured with your Honeycomb API key and knows where to send the telemetry data. The SDK batches this data efficiently, so it doesn't impact your app's performance.
...
**Audio:** "When users navigate between different pages in Meminator—like going from the homepage to the create-your-own-phrase-page—React Router handles these transitions. The SDK automatically creates spans for route changes, so you can track page load times, see which pages users visit most, and identify slow navigation patterns."
...
**Audio:** "While automatic instrumentation covers the basics, you can add custom spans for business-specific events. In Meminator, you might want to track how long image selection takes or which meme phrases are most popular.
